...
@route("/standard_1", method="POST")
def do_uploadtl():
    lines_img = cv2.imread("web/image_examples/1.png", 1)
...
@route("/standard_2", method="POST")
def do_uploadtl():
    lines_img = cv2.imread("web/image_examples/2.png", 1)
    lines_img = np.array(cv2.resize(lines_img, (512,512)))
    lines_img = cv2.adaptiveThreshold(cv2.cvtColor(lines_img, cv2.COLOR_BGR2GRAY), 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY, blockSize=9, C=2)
    lines_img = cv2.merge((lines_img,lines_img,lines_img,255 - lines_img))
    cnt = cv2.imencode(".png",lines_img)[1]
...
@route("/standard_3", method="POST")
def do_uploadtl():
    lines_img = cv2.imread("web/image_examples/3.png", 1)
    lines_img = np.array(cv2.resize(lines_img, (512,512)))
    lines_img = cv2.adaptiveThreshold(cv2.cvtColor(lines_img, cv2.COLOR_BGR2GRAY), 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY, blockSize=9, C=2)
    lines_img = cv2.merge((lines_img,lines_img,lines_img,255 - lines_img))
    cnt = cv2.imencode(".png",lines_img)[1]
...
def do_uploadtl():
    ...
    lines_img = cv2.imdecode(np.fromstring(img.file.read(), np.uint8), cv2.IMREAD_COLOR)
    lines_img = np.array(cv2.resize(lines_img, (512,512)))
    lines_img = cv2.adaptiveThreshold(cv2.cvtColor(lines_img, cv2.COLOR_BGR2GRAY), 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY, blockSize=9, C=2)
    lines_img = cv2.merge((lines_img,lines_img,lines_img,255 - lines_img))
    cnt = cv2.imencode(".png",lines_img)[1]
